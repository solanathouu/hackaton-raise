...
export class LiveCoordinatorEngine extends EventTarget {
  ...
  _connect() {
    ...
    this.socket.on("connect", () => {
      ...
      this.emit("decision", { kind: "neutral", title: "Coordinator connected", body: "Live brain view active.", tone: "neutral" });
    });
    this.socket.on("disconnect", () => {
      this.emit("connection", { connected: false });
      this.emit("decision", { kind: "warning", title: "Coordinator disconnected", body: "View is no longer fed.", tone: "danger" });
    });
  }
  ...
  completeMove(agentId, role, incidentId, targetZone) {
    ...
    if (role === "backfill") {
      ...
      this.emit("decision", {
        ...
        body: `${agent.name} is covering the zone. The gap is closed.`, tone: "success",
      });
