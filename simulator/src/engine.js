...
export class DispatchEngine extends EventTarget {
  ...
  setConstraint(name, enabled) {
    ...
    this.emit("decision", {
      ...
      body: enabled
        ? "The Grand Huit RCP pair is protected except as a last resort."
        : "The Grand Huit RCP pair is available again.",
      tone: "neutral"
    });
  }
  ...
  triggerIncident(zoneId, options = {}) {
    ...
    this.emit("speak", {
      speaker: "Conductor",
      text: `Voice report at ${zone.name}. Reading ${incident.type.replaceAll("_", " ")}. Required skill: ${formatSkills(incident.skillsNeeded)}.`
    });
  ...
  dispatchPrimary(incident, candidates, candidateIndex) {
    ...
    this.emit("decision", {
      ...
      body: `${agent.name} is the closest qualified responder (${formatSkills(incident.skillsNeeded)}). ${choice.createsGap ? `Leaving ${this.zone(sourceZone).name} creates a gap: backfill started.` : `Leaving ${this.zone(sourceZone).name} stays within minimums.`}`,
      tone: choice.createsGap ? "warning" : "success"
    });
    this.emit("assignment", { assignment, incident });
    this.emit("speak", {
      speaker: agent.name,
      text: `${agent.name}, copy. Person down at ${incident.zoneName}. On my way.`
    });
  ...
  timeoutPrimary(incident, candidates, candidateIndex, assignmentId) {
    ...
    this.emit("speak", {
      speaker: "Conductor",
      text: `No ack from ${agent.name}. Immediate re-route for this incident.`
    });
  ...
  ackPrimary(incident, assignmentId) {
    ...
    this.emit("speak", {
      speaker: "Conductor",
      text: `${agent.name} confirmed. ${agent.name}, proceed to ${incident.zoneName}.`
    });
  ...
  assignBackfill(incident, candidate, targetZoneId, hop) {
    ...
    this.emit("decision", {
      ...
      body: `${sourceName} → ${target.name}. ${candidate.safe ? "No second-order gap created." : "Creates a second-order gap; cascade continues."}`,
      tone: candidate.safe ? "success" : "warning"
    });
    this.emit("assignment", { assignment, incident });
    this.emit("speak", {
      speaker: "Conductor",
      text: `${agent.name}, move to ${target.name} as reinforcement. Close the coverage gap.`
    });
    this.emit("move", {
      agentId: agent.id,
      role: "backfill",
  ...
  completeMove(agentId, role, incidentId, targetZone) {
    ...
    if (role === "backfill") {
      ...
      this.emit("speak", {
        speaker: "Conductor",
        text: `Coverage at ${this.zone(targetZone).name} restored by ${agent.name}.`
      });
      this.emit("coverage", this.getCoverage(false));
      return;
    }
    ...
    if (role === "primary" && incident) {
      ...
      this.emit("decision", {
        ...
        body: "Ambulance corridor clearing while the responder stabilizes the scene.",
        tone: "hot"
      });
      this.emit("speak", {
        speaker: agent.name,
        text: `${agent.name} with the visitor. Preparing transfer to ambulance.`
      });
  ...
  transportToAmbulance(agentId, incidentId) {
    ...
    this.emit("decision", {
      kind: "ambulance",
      title: `Ambulance transfer open`,
      body: `${agent.name} escorts the patient to the service gate ambulance point.`,
      tone: "neutral"
    });
    this.emit("speak", {
      speaker: "Conductor",
  ...
  completeAmbulance(agentId, incidentId) {
    ...
    this.emit("decision", {
      ...
      body: `Patient handed off to ambulance. ${agent.name} is available at Parking.`,
      tone: "success"
    });
    this.emit("speak", {
      speaker: "Conductor",
